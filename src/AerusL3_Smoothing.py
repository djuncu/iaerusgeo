...
def Smoothing(cfg, fpath):
    ...
    dsets = dict(zip(SDS_NAMES, [ h5f[key] for key in SDS_NAMES ]))
    dset = h5f[SDS_NAME]
    nx, ny = dset.shape
    ...
    fdsets = dict(zip(SDS_NAMES, [ ravel(dsets[key][:]) for key in SDS_NAMES ]))
    ...
    for key, val in dset.attrs.items(): newdset.attrs[key] = val
