   ...
   Do NB = 1, N_Blocks
      ...
      Do Y = 1, Lines
         ...
         Do X = 1, MSGpixX
            ...
            if (aer_type_1 .eq. 9) aer_type_1  = 1
            if (aer_type_1 .eq. 10) aer_type_1  = 6
            if (aer_type_1 .eq. 11) aer_type_1  = 7
...
!            if (aer_type_2 .ne. 255) Then
            if (aer_type_2 .ne. -1) Then
               ...
               if (aer_type_2 .eq. 9) aer_type_2  = 1
               if (aer_type_2 .eq. 10) aer_type_2  = 6
               if (aer_type_2 .eq. 11) aer_type_2  = 7
...
!            if (aer_type_3 .ne. 255) Then
            if (aer_type_3 .ne. -1) Then
               ...
               if (aer_type_3 .eq. 9) aer_type_3  = 1
               if (aer_type_3 .eq. 10) aer_type_3  = 6
               if (aer_type_3 .eq. 11) aer_type_3  = 7
...
!                                                If (phi_sol_near .gt. two_pi) phi_sol_near = two_pi
                                 ...
                                 & ref_tol_near(i_smooth)*uv_near(i_smooth)*(sqrt(uv_near(i_smooth)**2+2.0*eta_sph+eta_sph**2)-uv_near(i_smooth))/eta_sph
                                                ...
                                                ref_s_near(i_smooth) = &
                                                   ...
                                                   & wspeed(1), wdir(1), I, model, ocean_flag, .False.),k_in_near(i_smooth,0:2))
                                                ref_s_near_(i_smooth) = &
                                                   & dot_product(brdfmodel(theta_sat_near, theta_sol_near, phi_sat_near, phi_sol_near, phi_del_near, &
                                                   & wspeed(1), wdir(1), I, model, ocean_flag, .True.),k_in_near_(i_smooth,0:2))
...
!                                                   & weight_near(i_smooth) * (1.0+((time_near(i_smooth)-1)/(N_scenes-1))**0.5)
                                                weight_near(i_smooth) = &
                                                   & weight_near(i_smooth) * (1.0+( ((1.0-EXP(-time_near(i_smooth)/2.0)) - (1.0-EXP(-1.0/2.0))) / &
                                                                                    ((1.0-EXP(-N_scenes/2.0))            - (1.0-EXP(-1.0/2.0)))    ))
...
!                                    w_ref_s = pi/xi-1.0
                                    w_ref_s_ = (xi-xi_trunc)/(pi-xi_trunc)
                                    w_ref_s = (pi-xi)/(pi-xi_trunc)
...
501                              k(3) = 0.
                                 ...
                                 Do While (cpt_tau .le. cpt_tau_max)
                                    ...
                                    CkI_tau(0, 0) = CkI_in(0, 0)*min(exp(tau_0), 50.)
                                    CkI_tau(1, 1) = CkI_in(1, 1)*min(exp(tau_0), 50.)
                                    CkI_tau(2, 2) = CkI_in(2, 2)*min(exp(tau_0), 50.)
                                    ! a priori when we go beyond a certain number of iterations
                                    if (cpt_tau .gt. cpt_tau_conv) then
                                       CkI_tau(0, 0) = CkI_tau(0, 0)*min(exp(tau_0), 50.)
                                       CkI_tau(1, 1) = CkI_tau(1, 1)*min(exp(tau_0), 50.)
                                       CkI_tau(2, 2) = CkI_tau(2, 2)*min(exp(tau_0), 50.)
                                    endif
