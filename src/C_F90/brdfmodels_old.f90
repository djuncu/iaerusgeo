...
Contains
  ...
  Function brdfmodel(theta_obs, theta_sun, phi_obs, phi_sun, phi_del, wind_speed, phiw, I, model, ocean_flag)
    Implicit None

    Real, Intent(In)     :: theta_obs, theta_sun, phi_obs, phi_sun, phi_del, wind_speed, phiw
    Integer, Intent(In)  :: model, I
    Logical, Intent(In)  :: ocean_flag ! flag for land/water mask
    ...
    Else
       brdfmodel = brdfmodel_land(theta_obs, phi_del, theta_sun, model)
    Endif

  End Function brdfmodel

  Function brdfmodel_land(theta_obs, phi_del, theta_sun, model_land)
    Implicit None
    ...
    Select Case (model_land)
     ...
     Case (3)
       brdfmodel_land = lirosshotspot(theta_obs, phi_del, theta_sun)
     Case Default
  ...
  Function brdfmodel_aerosol(theta_obs, theta_sun, phi_obs, phi_sun, phi_del, wind_speed, phiw, I, model, ocean_flag, tau0, g_, ssa_, trans_coeff, eta, phFunc)
    Implicit None
    ...
    Else
      brdfmodel_aerosol = brdfmodel_land_aerosol(theta_obs, phi_del, theta_sun, model, tau0, g_, ssa_, trans_coeff, eta, phFunc)
    Endif

  End Function brdfmodel_aerosol

  Function brdfmodel_land_aerosol(theta_obs, phi_del, theta_sun, model_land, tau0, g_, ssa_, trans_coeff, eta, phFunc)
    Implicit None
    ...
    Select Case (model_land)
     ...
     Case (3)
       brdfmodel_land_aerosol = lirosshotspot_aerosol(theta_obs, phi_del, theta_sun, tau0, g_, ssa_, trans_coeff, eta, phFunc)
     Case Default
       print *, 'wrong BRDF model for land'  
       stop 
    End Select
 ...
 Function rtls(theta_obs, phi_del, theta_sun)
  ...
  Function lirosshotspot(theta_obs, phi_del, theta_sun)
    Implicit None
    ...
    Real, Parameter :: pi_half      = pi/2.
    Real, Parameter :: one_third    = 1./3.
    Real, Parameter :: phaseAng0    = 1.5/180.*pi
...
!    rtls_aerosol(3) =ssa_/4./cos_tsun/cos_tobs*p*exp_x*(1.+(7.-tau0)*tau0/5.)
  ...
  Function lirosshotspot_aerosol(theta_obs, phi_del, theta_sun, tau0, g_, ssa_, trans_coeff, eta, phFunc)
    Implicit None
